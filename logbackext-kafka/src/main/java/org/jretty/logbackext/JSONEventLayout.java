...
import org.jretty.logbackext.util.IPCollector;
import org.jretty.logbackext.util.LogIdGenerator;
...
public class JSONEventLayout extends LayoutBase<ILoggingEvent> {
	...
	public synchronized String doLayout(ILoggingEvent event) {
		...
		buf.append(COMMA);
		appendKeyValue(buf, "logid", LogIdGenerator.getCurrentLogId(), null);
		buf.append(COMMA);
		appendKeyValue(buf, "ip", IPCollector.getLocalIp(), null);
		buf.append(COMMA);
		...
		if (locationInfo) {
			...
			if (callerDataArray != null && callerDataArray.length > callerStackIdx) {
				buf.append(COMMA);
				buf.append("\"location\":{");
				StackTraceElement immediateCallerData = callerDataArray[callerStackIdx];
				appendKeyValue(buf, "class", immediateCallerData.getClassName(), null);
				buf.append(COMMA);
				appendKeyValue(buf, "method", immediateCallerData.getMethodName(), null);
				buf.append(COMMA);
				appendKeyValue(buf, "file", immediateCallerData.getFileName(), null);
				buf.append(COMMA);
				appendKeyValue(buf, "line", Integer.toString(immediateCallerData.getLineNumber()), null);
				buf.append("}");
			}
		}
